      ...
      {({ id, labelId }) => (
        <Flex gap="xs" align="center">
          <Popover open={open} onOpenChange={(v) => { if (disabled) return; setOpen(v); }}>
            {/* The trigger is the combobox itself and the clear affordance is its sibling, the
                shape every other select field in this package uses. Nesting a control inside a
                control leaves the keyboard and assistive technology to guess which of the two a
                press belongs to, and the guesses disagree. */}
            <PopoverTrigger asChild>
              <span
                ...
                className={cn(
                  "flex h-8 w-full min-w-0 flex-1 items-center gap-1 rounded-md border border-input bg-background px-3 text-sm",
              ...
              >
                {selectedOption ? (
                  ...
                  <span className="flex-1 truncate">{placeholder ?? t("field.selectTimezone")}</span>
                )}
                {value && !disabled && (
                  <button
                    type="button"
