...
export function TimezoneField({
  ...
  const { Popover, PopoverContent, Command, CommandInput, CommandList, CommandEmpty, CommandGroup, CommandItem } = useFlatUIComponents();
          ...
          <Popover open={open} onOpenChange={(v) => { if (disabled) return; setOpen(v); }}>
            ...
            >
              <span className="flex-1 truncate">
                {selectedOption ? selectedOption.label : (placeholder ?? t("field.selectTimezone"))}
              </span>
            </SelectFieldTrigger>
            <PopoverContent
