 ...
 * list that has to be kept in step with the directories the generators emit.
 */
export function metaIndexContent(importTranslations: boolean): string {
  const lines: string[] = [];
  if (importTranslations) lines.push('import "./translations";');
  lines.push(`export * from "${META_MODULE}";`);
  return lines.join("\n") + "\n";
}
