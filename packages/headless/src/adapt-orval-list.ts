import { buildSearchableParams, type SearchableListParams } from "./build-searchable-params";
import type { ListHook, ListHookResult } from "./list-types";
 ...
 *
 * @remarks
 * Handles the following conversions (via {@link buildSearchableParams}):
...
export function adaptOrvalList<T>(
  ...
  return (params?: Record<string, unknown>): ListHookResult<T> => {
    const apiParams = buildSearchableParams(params as SearchableListParams | undefined, {
      transformFilters: options?.transformFilters,
    });
