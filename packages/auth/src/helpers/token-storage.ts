...
export const REFRESH_EXPIRES_AT_KEY = "refresh_expires_at";

function resolveExpiresAt(value: string | number): number {
...
export function storeTokenPair(store: TokenStore, pair: TokenPair): void {
  ...
  if (pair.expiresAt) {
    store.set(EXPIRES_AT_KEY, String(resolveExpiresAt(pair.expiresAt)));
  } else if (pair.expiresIn) {
    const expiresAt = Date.now() + pair.expiresIn * 1000;
    store.set(EXPIRES_AT_KEY, String(expiresAt));
  }

  if (pair.refreshTokenExpiresAt) {
    store.set(
      REFRESH_EXPIRES_AT_KEY,
      String(resolveExpiresAt(pair.refreshTokenExpiresAt)),
    );
  }
}
