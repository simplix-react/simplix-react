...
function responseRefs(domain: ResolvedDomain): TypeRef[] {
  return domain.operations
    .map((operation) => operation.response)
    .filter((ref): ref is TypeRef => ref !== undefined);
}

function requestRefs(domain: ResolvedDomain): TypeRef[] {
  const refs: TypeRef[] = [];
 ...
 * of the shape a descendant is sent as, and TypeScript declares them in the ancestor.
 */
function reachableFrom(domain: ResolvedDomain, seeds: TypeRef[]): Set<string> {
