...
**마지막 도메인을 옮겨도 `spec`은 남는다.** 도메인의 `generated/`를 지우는 것과 설정에서 `spec`을
빼는 것은 다른 일이고, 이 설계는 앞엣것만 다룬다. `openapi` 명령의 일곱 단계가 OpenAPI를 파싱한
엔티티에 매달려 있기 때문이다 — 변경 감지, `crud.config.ts` 생성, 훅·mock 생성, 로케일 파일,
i18n 내려받기의 엔티티 목록, 스냅샷 저장, `.http` 파일. 이 가운데 훅과 mock만 메타 경로가 대신
만든다.
